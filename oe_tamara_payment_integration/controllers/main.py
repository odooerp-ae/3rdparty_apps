...
class PaymentTamaraController(http.Controller):
    ...
    @http.route(
        [success_url, failure_url, cancel_url], type="http", auth="public", csrf=False
    )
    def payment_checkout_tamara_return(self, *args, **kwargs):
        jomla_website_url = (
            request.env["ir.config_parameter"]
            .sudo()
            .get_param("jomla_website_url", "https://odoo.jomla.ae")
        )
        lang = "ar"
        try:
            tx = False
            _logger.info(
                "Tamara: Entering form_feedback with post data %s",
                pprint.pformat(kwargs),
            )
            tx = (
                request.env["payment.transaction"]
                .sudo()
                ._handle_notification_data("tamara", kwargs)
            )
            ...
            if tx.state == "cancel":
                return request.redirect(
                    f"{jomla_website_url}/{lang}/checkout/?payment_status=cancelled",
                    local=False,
                )

            if tx.state == "error":
                return request.redirect(
                    f"{jomla_website_url}/{lang}/checkout/?payment_status=failed",
                    local=False,
                )

            return request.redirect(
                f"{jomla_website_url}/{lang}/thank-you/?order_id={sale_order.id}",
                local=False,
            )
    ...
    @http.route(
        [notification_url], type="json", auth="public", methods=["POST"], csrf=False
    )
    ...
    def _validate_token(self, tx, token):
        ...
        if tx.provider_id.tamara_notification_token:
            try:
                data = jwt.decode(
                    token,
                    tx.provider_id.tamara_notification_token,
                    algorithms=["HS256"],
                )
                ...
                raise ValidationError(
                    _(
                        "Tamara: "
                        + _("Merchant Verification Token is failed:\n%s", str(e))
                    )
                )
        else:
            raise ValidationError(
    ...
    def _tamara_validate_notification(self, data):
        ...
        tx_sudo = (
            request.env["payment.transaction"]
            .sudo()
            .search([("reference", "=", data.get("order_reference_id"))])
        )
