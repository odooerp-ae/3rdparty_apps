...
from odoo.addons.oe_tamara_payment_integration.controllers.main import (
    PaymentTamaraController,
)
...
class PaymentTamaraConnect(models.Model):
    ...
    tamara_merchant_token = fields.Char()
    ...
    def _get_tamara_consumer_data(self, tamara_txn_values):
        ...
        consumer_data["phone_number"] = (
            tamara_txn_values.get("billing_partner_phone")
            and tamara_txn_values.get("billing_partner_phone")[:33]
        )
        consumer_data["email"] = (
            tamara_txn_values.get("billing_partner_email")
            and tamara_txn_values.get("billing_partner_email")[:129]
        )
    ...
    def _get_tamara_shipping_address(self, tamara_txn_values):
        ...
        shipping_address["line1"] = (
            tamara_txn_values.get("billing_partner").street
            and tamara_txn_values.get("billing_partner").street[:129]
        )
        shipping_address["line2"] = (
            tamara_txn_values.get("billing_partner").street2
            and tamara_txn_values.get("billing_partner").street2[:129]
        )
    ...
    def _get_tamara_shipping_amount(self, tamara_txn_values):
        ...
        # Extract the reference (assumes format "SOxxx-xxx")
        reference = tamara_txn_values.get("reference").split("-")[0]

        sale_order = (
            self.env["sale.order"].sudo().search([("name", "=", reference)], limit=1)
        )

        # Detect delivery lines reliably
        # shipping_cost = sum(
        #     line.price_total
        #     for line in sale_order.order_line
        #     if line.product_id
        #     and line.product_id.detailed_type == "service"
        #     and "delivery" in line.name.lower()
        # )
        ...
        shipping_amount[
            "currency"
        ] = sale_order.currency_id.name  # safer than tamara_txn_values.get("currency")
        return shipping_amount
    ...
    def _get_tamara_mechant_url(self, tamara_txn_values):
        ...
        merchant_urls["success"] = (
            str(urls.url_join(base_url, PaymentTamaraController.success_url))
            + f"?reference={reference}"
        )
        merchant_urls["failure"] = (
            str(urls.url_join(base_url, PaymentTamaraController.failure_url))
            + f"?reference={reference}"
        )
        merchant_urls["cancel"] = (
            str(urls.url_join(base_url, PaymentTamaraController.cancel_url))
            + f"?reference={reference}"
        )
        merchant_urls["notification"] = (
            str(urls.url_join(base_url, PaymentTamaraController.notification_url))
            + f"?reference={reference}"
        )
    ...
    #             line_item["sku"] = (
    #                 line.product_id.default_code or line.product_id.hs_code
    #             )
    ...
    def _get_tamara_items_detail(self, tamara_txn_values):
        ...
        for line in order.order_line:
            if line.product_id.type != "service":
                ...
                line_item["type"] = line.product_id.categ_id.name
                line_item["name"] = line.product_id.name
                line_item["sku"] = line.product_id.default_code or getattr(line.product_id, 'hs_code', '')
                line_item["quantity"] = int(line.product_uom_qty)
                line_item["total_amount"] = {
                    "amount": line.price_total,
                    "currency": tamara_txn_values.get("currency").name,
    ...
    def get_payment_type(self, headers, api_end, path, request_data):
        ...
        if not user_country_id:
            user_country_id = (
                self.company_id.country_id.code if self.company_id.country_id else ""
            )
        _logger.info("######## User Country Code %s ############", user_country_id)
        response = requests.get(
            url=api_end + payment_type,
            headers=headers,
            data=json.dumps({"country": user_country_id}),
            timeout=180,
        )
        return response
    ...
    def _tamara_send_request(self, request_data, method=None, path=None):
        ...
        api_end = (
            self.APIEND.get("test_url")
            if self.state == "test"
            else self.APIEND.get("live_url")
        )
        ...
        try:
            ...
            if method == "post":
                ...
                response = requests.get(
                    url=api_end + path, headers=HEADERS, timeout=180
                )
                _logger.info("########GET RESPONSE DATA##########%s", response.text)
                return response
        except Exception as e:
            _logger.warning("#DEBUG---TAMARA----Exception-----%r---------" % (e))
            raise UserError(e)
    ...
    def _tamara_make_request(self, request_data, method=None, path=None):
        ...
        if resp_data.get("success") == False:
            for error in resp_data.get("errors"):
                _logger.error("---Tamara-Err-Resp------%s", error)
    ...
    def _get_default_payment_method_id(self, code):
        ...
        return self.env.ref("oe_tamara_payment_integration.payment_tamara_connect").id
...
class PaymentTransactionTamara(models.Model):
    ...
    def _get_buyer_billing_address(self, iyzico_txn_values: dict) -> dict:
        ...
        buyer_billing_data = {
            ...
            "billing_partner_address": str(partner_invoice_id.street)
            + str(partner_invoice_id.street2),
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if not reference or not paymentStatus:
            error_msg = _(
                "Tamara: received data with missing reference (%s) or paymentStatus (%s)"
            ) % (
                reference,
                paymentStatus,
            )
        ...
        tx = self.search(
            [("reference", "=", reference), ("provider_code", "=", "tamara")]
        )
    ...
    def _tamara_form_get_invalid_parameters(self, data):
        ...
        order_details = self.provider_id._tamara_make_request(
    ...
    def _process_notification_data(self, data):
        ...
        if self.state == "done":
            _logger.warning(
                "Tamara: trying to validate an already validated tx (ref %s)"
                % self.reference
            )
            return True
    ...
    def check_payment_status_tamara(self, orderId, paymentStatus):
        if paymentStatus == "approved":
            ...
            req_path = f"/orders/{orderId}/authorise"
            auth_response = self.provider_id._tamara_send_request(
            ...
            error_msg = (
                "Tamara: Received unknown status for Tamara reference %s, set as error:  %s"
                % (
                    self.reference,
                    paymentStatus,
                )
    ...
    def _send_capture_request(self):
        ...
        capture_response = self.provider_id._tamara_make_request(
            request_data, method="post", path=req_path
        )
        capture_json_data = capture_response.get("data")
        if capture_json_data.get("capture_id"):
            capture_msg = (
                "Tamara: Transaction has been captured with capture id {}".format(
    ...
    def _send_void_request(self):
        ...
        req_path = f"/orders/{self.provider_reference}/cancel"
        cancel_response = self.provider_id._tamara_make_request(
            request_data, method="post", path=req_path
        )
