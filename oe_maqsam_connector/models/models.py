...
class MaqsamBaseApi(models.AbstractModel):
    ...
    def _get_maqsam_api_url(self, version='v2'):
        config_param = self.env['ir.config_parameter'].sudo()
        if version == 'v1':
            return config_param.get_param('maqsam.api.base_url.v1', 'https://api.maqsam.com/v1')
        else:
            return config_param.get_param('maqsam.api.base_url.v2', 'https://api.maqsam.com/v2')

    @api.model
    def _get_maqsam_portal_url(self):
        config_param = self.env['ir.config_parameter'].sudo()
        return config_param.get_param('maqsam.portal.base_url', 'https://portal.maqsam.com')
...
class MaqsamCall(models.Model):
    ...
    agent_email = fields.Char(related="contact_name.email", string='Agent Email', readonly=False)
    phone = fields.Char(related="contact_name.phone", string='Phone Number', readonly=False)
    caller = fields.Char(related="contact_name.mobile", string='Caller Number', readonly=False)
