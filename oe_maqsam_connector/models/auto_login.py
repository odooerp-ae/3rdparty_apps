...
class MaqsamAutoLogin(models.Model):
    ...
    def get_auth_token(self):
        ...
        try:
            ...
            if not token:
                _logger.error(
                    "Maqsam API returned no token for user %s. Response: %s",
                    self.user_email,
                    response.text,
                )
                raise UserError(
                    _(
                        "Failed to retrieve authentication token: Token not found in Maqsam API response."
                    )
                )
            ...
            _logger.exception(
                "Failed to retrieve Maqsam auth token for user %s", self.user_email
            )
            error_msg = (
                response.json().get("message", str(e))
    ...
    def auto_login(self):
        ...
        if not self.auth_token:
            try:
                ...
                raise UserError(
                    _(f"Failed to get authentication token for auto-login: {e}")
                )

        if not self.auth_token:
            raise UserError(
                _("No authentication token available to perform auto-login.")
            )
