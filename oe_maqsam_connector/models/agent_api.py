...
class MaqsamAgent(models.Model):
    ...
    identifier = fields.Char(string='Identifier', required=True, index=True, copy=False)
    name = fields.Char(string='Name')
    email = fields.Char(string='Email')
    active = fields.Boolean(string='Active')
    groups = fields.Char(string='Groups')
    outgoing_enabled = fields.Boolean(string='Outgoing Enabled')
    incoming_enabled = fields.Boolean(string='Incoming Enabled')
    whatsapp_enabled = fields.Boolean(string='WhatsApp Enabled')
    state = fields.Char(string='State')
    state_start_time = fields.Datetime(string='State Start Time')
    ...
    def fetch_agents(self, *args, **kwargs):
        ...
        try:
            ...
            raise UserError(_("Failed to connect to Maqsam API for agents: %s") % str(e))
        except Exception as e:
            _logger.exception("An unexpected error occurred during agent fetch request.")
            raise UserError(_(f"An unexpected error occurred while fetching agents: {e}"))
        ...
        if response.status_code == 200:
            ...
            existing_agents_map = {
                ...
                for agent in self.search_read([('identifier', 'in', [a.get('identifier') for a in agents_data if a.get('identifier')])], ['identifier'])
            }
            ...
            for agent_api_data in agents_data:
                ...
                if not identifier:
                    _logger.warning("Maqsam agent data missing 'identifier'. Skipping record: %s", agent_api_data)
                    continue

                state_info = agent_api_data.get('state', {})
                state_value = state_info.get('state') if isinstance(state_info, dict) else state_info
                state_timestamp = state_info.get('timestamp') if isinstance(state_info, dict) else None

                parsed_created_at = None
                if agent_api_data.get('createdAt'):
                    try:
                        parsed_created_at = datetime.strptime(agent_api_data['createdAt'], self.DATE_FORMAT)
                    except ValueError:
                        _logger.warning("Could not parse 'createdAt' ('%s') for agent %s. Skipping this field.", agent_api_data.get('createdAt'), identifier)

                parsed_state_start_time = None
                if state_timestamp:
                    try:
                        parsed_state_start_time = datetime.strptime(state_timestamp, self.DATE_FORMAT)
                    except ValueError:
                        _logger.warning("Could not parse 'state_start_time' ('%s') for agent %s. Skipping this field.", state_timestamp, identifier)
            ...
            raise UserError(_("Unauthorized access. Please check your Maqsam API credentials in Settings."))
        else:
            error_message = response.json().get('message', response.text)
            _logger.error("Failed to fetch agents. Status code: %s, Response: %s", response.status_code, response.text)
            raise UserError(_("Failed to fetch agents from Maqsam. Error: %s") % error_message)