...
class SalesForceMaster(models.Model):
    ...
    sf_client_id = fields.Char(string="Consumer Key", help="The client ID you obtain from the developer dashboard.")
    sf_client_secret = fields.Char(string="Consumer Secret", help="The client secret you obtain from the developer dashboard.")

    sf_auth_base_url = fields.Char(string="Authorization URL", default="https://login.salesforce.com/services/oauth2/authorize",
                                   help="User authenticate uri")
    sf_access_token_url = fields.Char(string="Authorization Token URL", default="https://login.salesforce.com/services/oauth2/token",
                                      help="Exchange code for refresh and access tokens")
    sf_request_token_url = fields.Char(string="Redirect URL", default="http://localhost:8069/get_auth_code_from_sf",
                                       help="One of the redirect URIs listed for this project in the developer dashboard.")
    sf_url = fields.Char(string="Instance URL", default="https://", help="SalesForce API URIs, use access token to call SalesForce API's")

    # used for api calling, generated during authorization process.
    sf_auth_code = fields.Char(string="Auth Code")
    sf_access_token = fields.Char(string="Access Token", help="The token that must be used to access the SALESFORCE API.")
    sf_refresh_token = fields.Char(string="Refresh Token")

    account_lastmodifieddate = fields.Datetime(string="Account Last Modified Time :- ", default=_default_update_datetime)
    contact_lastmodifieddate = fields.Datetime(string="Contact Last Modified Time :- ", default=_default_update_datetime)
    product_lastmodifieddate = fields.Datetime(string="Product Last Modified Time :- ", default=_default_update_datetime)
    quote_lastmodifieddate = fields.Datetime(string="Quotation Last Modified Time :- ", default=_default_update_datetime)
    order_lastmodifieddate = fields.Datetime(string="Sale Order Last Modified Time :- ", default=_default_update_datetime)
    lead_lastmodifieddate = fields.Datetime(string="Lead Last Modified Time :- ", default=_default_update_datetime)
    opportunity_lastmodifieddate = fields.Datetime(string="Opportunity Last Modified Time :- ", default=_default_update_datetime)
    contract_lastmodifieddate = fields.Datetime(string="Contract Last Modified Time :- ", default=_default_update_datetime)
    event_lastmodifieddate = fields.Datetime(string="Event Last Modified Time :- ", default=_default_update_datetime)
    task_lastmodifieddate = fields.Datetime(string="Task Last Modified Time :- ", default=_default_update_datetime)

    company_id = fields.Many2one('res.company', string="Company", required=True, default=lambda self: self.env.company.id, readonly=True, store=True)
    import_limit = fields.Integer(string="Import Limit", default=50)
    export_limit = fields.Integer(string="Export Limit", default=50)
    is_default_instance = fields.Boolean(string="Is default Instance for Cron")
    salesforce_instance_line_ids = fields.One2many('salesforce.instance.line', 'salesforce_instance_id', string="Salesforce Instance Line")
    ...
    def create(self, vals):
        ...
        if self.search_count([('company_id', '=', result.company_id.id)]) > 1:
            raise ValidationError("Can't create two instance for the same company")

        if vals.get('is_default_instance'):
            if self.search_count([('is_default_instance', '=', True)]) > 1:
                raise ValidationError("Can't create two instance as default")
    ...
    def write(self, vals):
        if vals.get('company_id'):
            if self.search_count([('company_id', '=', vals.get('company_id'))]) >= 1:
                raise ValidationError("Can't Update two instance for the same company")
    ...
    def refresh_salesforce_token_from_access_token(self, is_cron=False):
        ...
        refresh_token_response = requests.post(sf_url + '/services/oauth2/token', data=refresh_token_data, headers=headers)
        if refresh_token_response.status_code == 200:
            ...
            if not is_cron:
                ...
                _logger.error("We got a issue !!!! Desc : {}".format(refresh_token_response.text))
    ...
    def sf_sendMessage(self, message):
        view_ref = self.env['ir.model.data']._xmlid_to_res_id('oe_salesforce_connector.salseforce_message_wizard_form')
        if view_ref:
            return {
    ...
    def import_sf_contacts(self):
        if self.sf_access_token:
            ...
            if self.import_limit:
                query_url = "/services/data/v40.0/query/?q=select Id, LastModifiedDate from contact where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                    sf_modified_date, self.import_limit)
            else:
                query_url = "/services/data/v40.0/query/?q=select Id, LastModifiedDate from contact where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                    sf_modified_date)

            contact_data = requests.request('GET', self.sf_url + query_url, headers=headers)
                ...
                if contact_parsed_data.get('records'):
                    for contact in contact_parsed_data.get('records'):
                        try:
                            ...
                            if result:
                                self.contact_lastmodifieddate = self.convert_sfdate_toodoo(
        ...
        try:
            if self.sf_access_token:
                ...
                if product_partner != False and not type(product_partner) == type(dummy_dict) and is_account == False:
                    endpoint = ""
                    if self.import_limit:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Contact where Id='{}' LIMIT {}".format(
                            product_partner,
                            self.import_limit)
                    else:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Contact where Id='{}'".format(
                            product_partner)
                    data = requests.request('GET', self.sf_url + endpoint, headers=headers)
                elif is_account and product_partner != False and not type(product_partner) == type(dummy_dict):
                    endpoint = ""
                    if self.import_limit:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Account where Id='{}' LIMIT {}".format(
                            product_partner,
                            self.import_limit)
                    else:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Account where Id='{}'".format(
                            product_partner)
                    ...
                    endpoint = ""
                    if self.import_limit:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Account LIMIT {}".format(
                            product_partner, self.import_limit)
                    else:
                        endpoint = "/services/data/v40.0/query/?q=select Id from Account"
    ...
    def getSfProductPrice(self, sf_id):
        if self.sf_access_token:
            ...
            query_url = ""
            if self.import_limit:
                query_url = "/services/data/v40.0/query/?q=select UnitPrice from pricebookentry where Product2Id='{}' LIMIT {}".format(
                    str(sf_id),
                    self.import_limit)
            else:
                query_url = "/services/data/v40.0/query/?q=select UnitPrice from pricebookentry where Product2Id='{}'".format(
                    str(sf_id))

            data = requests.request('GET', self.sf_url + query_url, headers=headers)
            if data.status_code:
    ...
    def import_sf_products(self, is_from_cron=False):
        ...
        if self.import_limit:
            url = "/services/data/v40.0/query/?q=select Id from Product2 where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                sf_modified_date, self.import_limit)
        else:
            url = "/services/data/v40.0/query/?q=select Id from Product2 where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                sf_modified_date)

        data = requests.request('GET', self.sf_url + url, headers=headers)
        ...
        try:
            if self.sf_access_token:
                ...
                query_url = ""
                if self.import_limit:
                    query_url = "/services/data/v40.0/query/?q=select Id from Product2 where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                        sf_modified_date, self.import_limit)
                else:
                    query_url = "/services/data/v40.0/query/?q=select Id from Product2 where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                        sf_modified_date)

                data = requests.request('GET', self.sf_url + query_url, headers=headers)
                file_data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from ContentDocument", headers=headers)
    ...
    def getSfQuoteLineData(self, quote_id=None, quote_dict=None, is_so=False, so_id=None, is_cron=False):
        if self.sf_access_token:
            headers = self.get_sf_headers()
            ...
            if self.import_limit:
                endpoint += 'LIMIT {}'.format((str(self.import_limit)))
                                ...
                                if data.status_code == 200:
                                    if data.text:
                                        ...
                                        if parsed_data.get('Description'):
                                            quote_line_dict['name'] = parsed_data.get('Description')
                                        if quote_dict:
    ...
    def import_sf_quote(self, is_from_cron=False):
        ...
        if self.sf_access_token:
            ...
            query_url = ""
            if self.import_limit:
                query_url = "/services/data/v40.0/query/?q=select Id from quote where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                    sf_modified_date, self.import_limit)
            else:
                query_url = "/services/data/v40.0/query/?q=select Id from quote where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                    sf_modified_date)
                    ...
                    if recs:
                        if recs.get('records'):
                            for r in recs.get('records'):
                                ...
                                if quote_data:
                                    if not quote_data.get('OpportunityId') and not quote_data.get(
                                            'ContactId'):
                                        _logger.info(
                                    ...
                                    if billing:
                                        ...
                                        quote_dict['x_order_billing_address'] = billing_address
                                    if shipping:
                                        shipping_address = "\n".join([
                                        ...
                                        quote_dict['x_order_shipping_address'] = shipping_address

                                    quote_dict['x_salesforce_ref'] = quote_data.get('QuoteNumber')
                                    quote_dict['note'] = quote_data.get('Description')
                                    quote_dict['validity_date'] = quote_data.get('ExpirationDate')
                                    quote_dict['x_salesforce_quote_name'] = quote_data.get('Name')
                                    ...
                                    if quote_data.get('OpportunityId') and not quote_data.get(
                                            'ContactId') and not quote_data.get(
                                        'AccountId') and not is_from_cron:
                                    ...
                                    if quote_dict:
                                        ...
                                        if not quote_exists:
                                            if quote_data.get('CurrencyIsoCode'):
                                                currency_code = quote_data.get('CurrencyIsoCode')
                                                pricelist_id = self.get_pricelist(currency_code)
                                                if pricelist_id:
                                                    quote_dict['pricelist_id'] = pricelist_id.id
                                            quote_create = self.env['sale.order'].sudo().create(quote_dict)
                                            if quote_create:
                                                shipping_cost = quote_data.get('ShippingHandling')
                                                if shipping_cost:
                                                    self.run_shipping_cost_process(quote_create, shipping_cost)
                                                else:
                                                    self.run_shipping_cost_process(quote_create, 0)
                                                self._process_attachments(r.get('Id'), quote_create.id)
                                            ...
                                            if quote_exists and quote_exists.state == 'draft':
                                                if quote_data.get('CurrencyIsoCode'):
                                                    currency_code = quote_data.get('CurrencyIsoCode')
                                                    pricelist_id = self.get_pricelist(currency_code)
                                                    if pricelist_id:
                                            ...
                                            shipping_cost = quote_data.get('ShippingHandling')
                                            if shipping_cost:
                                                self.run_shipping_cost_process(quote_exists, shipping_cost)
                                            else:
                                                self.run_shipping_cost_process(quote_exists, 0)
                                            self._process_attachments(r.get('Id'), quote_exists.id)
                                            if rec_update_so:
                                            ...
                                            if sale_order_line_ids:
                                                for line_id in sale_order_line_ids:
                                                    if line_id.x_salesforce_id:
                                                        odoo_sale_order_line.append(line_id.x_salesforce_id)
    ...
    def get_pricelist(self, currency_iso):
        pricelist = self.env['product.pricelist'].search([('currency_id.name', '=', currency_iso)], limit=1)
        if pricelist:
            return pricelist
        else:
            return None
    ...
    def run_shipping_cost_process(self, quote_id, shipping_cost):
        line_exists = quote_id.order_line.filtered(lambda line: line.sf_shipping_cost_line)
        if line_exists and shipping_cost == 0:
            line_exists.unlink()
        elif shipping_cost != 0:
            shipping_product_id = self.env.ref('oe_salesforce_connector.product_shipping_handling')
        ...
        if attachments:
            for file_name, file_data, sf_doc_id in attachments:
                ...
                if not existing_attachment:
                    ...
                    _logger.info(f"Attachment {file_name} added to Sale Order {sale_order_id}")
    ...
    def get_sf_quote_attachments(self, quote_id):
        ...
        response = requests.get(self.sf_url + query_url, headers=headers)
        ...
        if response.status_code in (200, 201):
            ...
            for record in content_links.get("records", []):
                ...
                content_response = requests.get(self.sf_url + content_query, headers=headers)
                ...
                if content_response.status_code in (200, 201):
                    ...
                    for content in content_data.get("records", []):
                        ...
                        file_response = requests.get(self.sf_url + file_data_url, headers=headers)
                        if file_response.status_code in (200, 201):
                            file_data = file_response.content
                            attachment_list.append((file_name, file_data, content.get('Id')))
                        else:
            ...
            if data.status_code == 200:
                if data.text:
                    ...
                    if recs:
                        cust_name = recs.get('Name')
                        # cust_exists = self.env['res.partner'].search([('display_name', '=', cust_name)], limit=1)
                        cust_exists = self.env['res.partner'].search([('x_salesforce_id', '=', recs.get('AccountId'))],
                                                                     limit=1)
                        if cust_exists:
                            return cust_exists.id
                        else:
    ...
    def import_sf_so(self, is_from_cron=False):
        ...
        if self.sf_access_token:
            ...
            endpoint = ""
            if self.import_limit:
                endpoint = "/services/data/v39.0/query/?q=select Id from order where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                    sf_modified_date, self.import_limit)
            else:
                endpoint = "/services/data/v39.0/query/?q=select Id from order where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                    sf_modified_date)
                        ...
                        for rec in parsed_data.get('records'):
                            ...
                            if so_data:
                                ...
                                so_line_data = self.getSfQuoteLineData(is_so=True, so_id=str(rec.get('Id')),
                                                                       quote_dict=so_data, is_cron=is_from_cron)
                                if so_line_data:
    ...
    def create_odoo_sf_contact_dictionary(self, sf_contact_info):
        ...
        if sf_contact_info.get('MailingCountry', False):
            if sf_contact_info.get('MailingCountry') in ['usa', 'USA', 'us', 'US', 'United States of America',
                                                         'united states of america',
                                                         'united states', 'United States']:
                country_id = self.env['res.country'].search([('code', 'like', 'US')], limit=1)
                partner_dict['country_id'] = country_id.id
            else:
                country_id = self.env['res.country'].search(
                    ['|', ('name', '=', sf_contact_info.get('MailingCountry')),
                     ('code', '=', sf_contact_info.get('MailingCountry'))], limit=1)
                if country_id:
                    partner_dict['country_id'] = country_id.id
                else:
        ...
        if sf_contact_info.get('MailingState', False):
            ...
            if country_id:
                state_id = self.env['res.country.state'].search(
                    ['|', ('name', '=', sf_state), ('code', '=', sf_state),
                     ('country_id', '=', country_id.id)], limit=1)
            else:
                state_id = self.env['res.country.state'].search(['|', ('name', '=', sf_state), ('code', '=', sf_state)],
                                                                limit=1)
            if state_id:
                partner_dict['state_id'] = state_id.id
            elif sf_state and partner_dict.get('country_id', False):
                state_id = self.env['res.country.state'].sudo().create({
                    'name': sf_state,
                    'code': sf_state,
    ...
    def create_odoo_sf_company_dictionary(self, sf_account_info):
        ...
        country_id = False
        if country:
            if country in ['usa', 'USA', 'us', 'US', 'United States of America', 'united states of america',
                           'united states', 'United States']:
                country_id = self.env['res.country'].search([('code', 'like', 'US')], limit=1)
                partner_dict['country_id'] = country_id.id
            else:
                country_id = self.env['res.country'].search(['|', ('name', '=', country), ('code', '=', country)],
                                                            limit=1)
                if country_id:
                    partner_dict['country_id'] = country_id.id
                else:
    ...
    def import_sf_accounts(self, is_cron=False):
        if self.sf_access_token:
            ...
            query_url = ""
            if self.import_limit:
                query_url = '''/services/data/v40.0/query/?q=select Id, LastModifiedDate from account where LastModifiedDate > {} ORDER BY LastModifiedDate LIMIT {}'''.format(
                    sf_modified_date, self.import_limit)
            else:
                query_url = '''/services/data/v40.0/query/?q=select Id, LastModifiedDate from account where LastModifiedDate > {} ORDER BY LastModifiedDate'''.format(
                    sf_modified_date)

            company_data = requests.request('GET', self.sf_url + query_url, headers=headers)
                    ...
                    for i, account in enumerate(acc_parsed_data.get('records')):
                        ...
                        try:
                            ...
                            if result:
                                ...
                                self.account_lastmodifieddate = self.convert_sfdate_toodoo(
    ...
    def import_sf_lead(self, is_from_cron=False):
        try:
            if self.sf_access_token:
                ...
                endpoint = ""
                if self.import_limit:
                    endpoint = "/services/data/v40.0/query/?q=select Id from Lead where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                        sf_modified_date, self.import_limit)
                else:
                    endpoint = "/services/data/v40.0/query/?q=select Id from Lead where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                        sf_modified_date)
                        ...
                        if recs:
                            for rec in recs:
                                ...
                                if lead_read.get('Address'):
                                    ...
                                    country_id = False
                                    if country:
                                        if country in ['usa', 'USA', 'us', 'US', 'United States of America',
                                                       'united states of america',
                                                       'united states', 'United States']:
                                            country_id = self.env['res.country'].search([('code', 'like', 'US')],
                                                                                        limit=1)
                                            ...
                                            if country_id:
                                                ...
                                                code_exist = self.env['res.country'].search([('code', '=', code)])
                                                if code_exist:
                                                    lead_dict['country_id'] = code_exist.id
                                                else:
                                                    country_id = self.env['res.country'].sudo().create(
                                                        {'name': country, 'code': code})
                                                    lead_dict['country_id'] = country_id.id
                                ...
                                if lead_dict:
                                    ...
                                    self.lead_lastmodifieddate = self.convert_sfdate_toodoo(
    ...
    def import_sf_opportunity(self, is_from_cron=False):
        try:
            if self.sf_access_token:
                ...
                endpoint = ""
                if self.import_limit:
                    endpoint = "/services/data/v40.0/query/?q=select Id from Opportunity where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                        sf_modified_date, self.export_limit)
                else:
                    endpoint = "/services/data/v40.0/query/?q=select Id from Opportunity where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                        sf_modified_date)
                            ...
                            for rec in recs:
                                ...
                                if opportunity_read.get('StageName'):
                                    crm_stage = self.env['crm.stage'].search(
                                        [('name', '=', opportunity_read.get('StageName'))], limit=1)
    ...
    def import_sf_contract(self, is_from_cron=False):
        try:
            if self.sf_access_token:
                ...
                endpoint = ""
                if self.import_limit:
                    endpoint = "/services/data/v40.0/query/?q=select Id from Contract where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                        sf_modified_date, self.import_limit)
                else:
                    endpoint = "/services/data/v40.0/query/?q=select Id from Contract where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                        sf_modified_date)
                data = requests.request('GET', self.sf_url + endpoint, headers=headers)
                if data.status_code in (200, 201):
                    if data:
                        ...
                        if recs:
                            for rec in recs:
                                ...
                                if event_read.get('StartDateTime'):
                                    result1 = datetime.strptime(event_read.get('StartDateTime')[0:19],
                                                                "%Y-%m-%dT%H:%M:%S")
                                    event_dict['start'] = result1
                                if event_read.get('EndDateTime'):
                                    result2 = datetime.strptime(event_read.get('EndDateTime')[0:19],
                                                                "%Y-%m-%dT%H:%M:%S")
                                    event_dict['stop'] = result2
                                if event_dict:
                                    self.create_sf_Event(event_dict, event_read.get('Id'))
                                    self.event_lastmodifieddate = self.convert_sfdate_toodoo(
    ...
    def import_sf_activity(self, is_from_cron=False):
        try:
            if self.sf_access_token:
                ...
                endpoint = ""
                if self.import_limit:
                    endpoint = "/services/data/v40.0/query/?q=select Id from Task where (LastModifiedDate > {}) ORDER BY LastModifiedDate LIMIT {}".format(
                        sf_modified_date, self.import_limit)
                else:
                    endpoint = "/services/data/v40.0/query/?q=select Id from Task where (LastModifiedDate > {}) ORDER BY LastModifiedDate".format(
                        sf_modified_date)
                            ...
                            for rec in recs:
                                ...
                                if activity_read.get('WhoId'):
                                    ...
                                    # activity_dict['res_id'] = res_partner.id
                                        ...
                                        if not is_from_cron:
                                            raise UserError(
                                                _("WhoId Not found in the record please add it first for id " + activity_read.get(
                                                    'Id')))
                                        else:
                                            _logger.error(
                                ...
                                if activity_read.get('WhatId'):
                                    ...
                                    # activity_dict['parent_id'] = res_partner.id
                                        ...
                                        if not is_from_cron:
                                            raise UserError(
                                                _("WhatId Not found in the record please add it first for id1 " + activity_read.get(
                                                    'Id')))
                                        else:
                                            _logger.error(
                                    ...
                                    if not is_from_cron:
                                        raise UserError(
                                            _("WhatId Not found in the record please add it first for id2 " + activity_read.get(
                                                'Id')))
                                    else:
                                        _logger.error(
                                ...
                                if activity_read.get('WhoId') and activity_read.get('WhatId'):
                                    ...
                                    # activity_dict['res_id'] = res_partner.id
                                        ...
                                        if not is_from_cron:
                                            raise UserError(
                                                _("WhatId Not found in the record please add it first for id3 " + activity_read.get(
                                                    'Id')))
                                        else:
                                            _logger.error(
                                    ...
                                    if not is_from_cron:
                                        raise UserError(
                                            _("WhatId Not found in the record please add it first for id4 " + activity_read.get(
                                                'Id')))
                                    else:
                                        _logger.error(
...
class SalesForceInstanceLine(models.Model):
    ...
    type = fields.Selection([
        ...
        ('contract', 'Contracts'),
        ('event', 'Events'),
        ('task', 'Tasks')], string="Type")
    date_time = fields.Datetime(string="Date & Time")
    state = fields.Selection([('success', 'Success'), ('error', 'Error'), ('nothing', 'Nothing')], string="State")
