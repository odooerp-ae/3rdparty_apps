...
class SFContract(models.Model):
    ...
    def activate_contract(self):
        ...
        try:
            res = requests.request('PATCH', sf_config.sf_url + endpoint, headers=headers, data=payload)
            if res.status_code == 200:
                _logger.info("Contract activated successfully in Salesforce.")
                return True
            else:
                _logger.error("Failed to activate the contract in Salesforce. Status code: %s", res.status_code)
                return False

        except Exception as e:
            _logger.error("An error occurred while activating the contract in Salesforce: %s", e)
            return False
