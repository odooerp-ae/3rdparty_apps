...
class MailActivity(models.Model):
    ...
    def exportActivity_to_sf(self, is_from_cron=False):
        ...
        if self.sf_status:
            activity_dict["Status"] = dict(self._fields["sf_status"].selection).get(
                self.sf_status
            )
        if self.sf_status:
            activity_dict["Priority"] = dict(self._fields["priority"].selection).get(
                self.priority
            )
    ...
    def _scheduler_export_activity_to_sf(self):
        ...
        for activity in activities:
            try:
                ...
                _logger.error(
                    "Oops Some error in  exporting Activity to SALESFORCE %s", e
                )
