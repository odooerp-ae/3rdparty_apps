...
class SaleOrderCust(models.Model):
    ...
    def unlink(self):
        ...
        if sf_config.sf_access_token:
            ...
            for record in self:
                if (record.x_salesforce_id or record.sale_order_salesforce_id) and (record.state == 'draft' or record.state == 'cancel'):
                    endpoint = "/services/data/v52.0/sobjects/Quote/" + str(record.x_salesforce_id)
                    ...
                    if res.status_code == 200 or res.status_code == 201 or res.status_code == 204:
                        _logger.info('Quote is deleted from SALESFORCE===========================')

                    if res.status_code == 404 or res.status_code == 400:
                        endpoint = "/services/data/v52.0/sobjects/Order/" + str(record.sale_order_salesforce_id)
                        res = requests.request('DELETE', sf_config.sf_url + endpoint, headers=headers)
                        if res.status_code == 200 or res.status_code == 201 or res.status_code == 204:
                            _logger.info('Order is deleted from SALESFORCE===========================')
        return super(SaleOrderCust, self).unlink()
        ...
        if sf_access_token:
            ...
            if self.x_salesforce_id:
                ...
                if res.status_code in (200, 201, 204):
                    ...
                    _logger.info('Quotation is updated in salesforce %s', self.x_salesforce_id)
                ...
                if res.status_code in (200, 201):
                    ...
                    _logger.info('Quotation is created in salesforce %s', parsed_resp.get('id'))
    ...
    def sendQuoteLineDataToSf(self, quote_line_dict, line_sf_id, line_id, is_cron=False):
        if is_cron:
            sf_config = self.env['salesforce.instance'].sudo().search([('is_default_instance', '=', True)], limit=1)
        else:
            sf_config = self.env['salesforce.instance'].sudo().search([('company_id', '=', self.env.company.id)], limit=1)
            ...
            if self.order_line:
                for line in self.order_line:
                    if line.x_salesforce_id and line.quotation_order_line_updated:
                        delete_quotation_line = self.delete_quotation_line_to_salesforce(line, line.x_salesforce_id, is_cron=is_from_cron)
                        if delete_quotation_line:
                            if line.product_id and line.product_id.x_salesforce_id:
                                ...
                                line.product_id.exportProduct_to_sf(is_from_cron=is_from_cron)
                                quote_line_dict = {
                                    'Product2Id': line.product_id.x_salesforce_id,
                                    'Quantity': line.product_uom_qty,
                                    'UnitPrice': line.price_unit,
                            ...
                            salesforce_pbe = ''
                            pricebook_id = ''
                            quote_data = requests.request('GET', sf_config.sf_url + "/services/data/v40.0/query/?q=select Pricebook2Id from Quote where Id = '{}'".format(self.x_salesforce_id),
                                                          headers=headers)
                            if quote_data.text:
                                quote_data = json.loads(str(quote_data.text))
                                pricebook_id = quote_data.get('records')[0].get('Pricebook2Id')
                            PricebookEntryData = requests.request('GET', sf_config.sf_url + "/services/data/v40.0/query/?q=select Id,UnitPrice from PricebookEntry where  Product2Id='{}'".format(
                                quote_line_dict['Product2Id']), headers=headers)
                            ...
                            if quote_line_dict:
                                result = self.sendQuoteLineDataToSf(quote_line_dict, line_sf_id, line_id, is_cron=is_from_cron)
                            self.x_salesforce_exported = True
                        ...
                        PricebookEntryData = requests.request('GET',
                                                              sf_config.sf_url + "/services/data/v40.0/query/?q=select Id,UnitPrice from PricebookEntry where  Product2Id='{}'".format(
                                                                  quote_line_dict['Product2Id']), headers=headers)
                        if PricebookEntryData.text:
                            pricebookentry_data = json.loads(str(PricebookEntryData.text))
                            for pricebook in pricebookentry_data.get('records'):
    ...
    def delete_quotation_line_to_salesforce(self, line_id, sf_id, is_cron=False):
        ...
        if sf_config.sf_access_token:
            ...
            if sf_access_token:
                ...
                res = requests.request('DELETE', sf_config.sf_url + endpoint, headers=headers)
                if res.status_code == 200 or res.status_code == 201 or res.status_code == 204:
                    _logger.info('Quotation order line is deleted from SALESFORCE ===========================')
    ...
    def _scheduler_export_quotes_to_sf(self):
        ...
        if sf_config.export_limit:
            quotes = self.search([('state', 'in', ('draft', 'sent'))], limit=sf_config.export_limit)
        else:
