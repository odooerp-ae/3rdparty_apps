...
class ResCompany(models.Model):
    ...
    sf_access_token = fields.Char(
        "Access Token", help="The token that must be used to access the SALESFORCE API."
    )
    sf_refresh_token = fields.Char("Refresh Token")

    account_lastmodifieddate = fields.Datetime(
        "Account Last Modified Time :- ", default=_default_update_datetime
    )
    contact_lastmodifieddate = fields.Datetime(
        "Contact Last Modified Time :- ", default=_default_update_datetime
    )
    product_lastmodifieddate = fields.Datetime(
        "Product Last Modified Time :- ", default=_default_update_datetime
    )
    quote_lastmodifieddate = fields.Datetime(
        "Quotation Last Modified Time :- ", default=_default_update_datetime
    )
    order_lastmodifieddate = fields.Datetime(
        "Sale Order Last Modified Time :- ", default=_default_update_datetime
    )
    lead_lastmodifieddate = fields.Datetime(
        "Lead Last Modified Time :- ", default=_default_update_datetime
    )
    opportunity_lastmodifieddate = fields.Datetime(
        "Opportunity Last Modified Time :- ", default=_default_update_datetime
    )
    contract_lastmodifieddate = fields.Datetime(
        "Contract Last Modified Time :- ", default=_default_update_datetime
    )
    event_lastmodifieddate = fields.Datetime(
        "Event Last Modified Time :- ", default=_default_update_datetime
    )
    task_lastmodifieddate = fields.Datetime(
        "Task Last Modified Time :- ", default=_default_update_datetime
    )
    ...
    def _scheduler_salesforce_login_aunthetication(self):
        company_id = (
            self.env["res.users"]
            .search([("id", "=", self.env.user.id)], limit=1)
            .company_id
        )
    ...
    def fetch_sf_cust_details(self, sf_id, is_account=False):
        ...
        if self.sf_access_token:
            ...
            if is_account:
                data = requests.request(
                    "GET",
                    self.sf_url + "/services/data/v40.0/sobjects/account/" + str(sf_id),
                    headers=headers,
                    timeout=180,
                )
            else:
                data = requests.request(
                    "GET",
                    self.sf_url + "/services/data/v40.0/sobjects/Contact/" + str(sf_id),
                    headers=headers,
                    timeout=180,
                )
            if data.status_code == 200:
    ...
    def import_sf_contacts(self):
        if self.sf_access_token:
            ...
            contact_data = requests.request(
                "GET",
                self.sf_url
                + f"""/services/data/v40.0/query/?q=select Id, LastModifiedDate from contact where (LastModifiedDate > {sf_modified_date}) ORDER BY LastModifiedDate""",
                headers=headers,
                timeout=180,
            )
            ...
            if self.sf_access_token:
                ...
                if (
                    ...
                    data = requests.request(
                        "GET",
                        self.sf_url
                        + f"/services/data/v40.0/query/?q=select Id from Contact where Id='{product_partner}'",
                        headers=headers,
                        timeout=180,
                    )
                elif (
                    ...
                    data = requests.request(
                        "GET",
                        self.sf_url
                        + f"/services/data/v40.0/query/?q=select Id from Account where Id='{product_partner}'",
                        headers=headers,
                        timeout=180,
                    )
                elif is_account:
                    ...
                    data = requests.request(
                        "GET",
                        self.sf_url
                        + "/services/data/v40.0/query/?q=select Id from Contact",
                        headers=headers,
                        timeout=180,
                    )
    ...
    def getSfProductPrice(self, sf_id):
        if self.sf_access_token:
            ...
            data = requests.request(
                "GET",
                self.sf_url
                + f"/services/data/v40.0/query/?q=select UnitPrice from pricebookentry where Product2Id='{str(sf_id)}'",
                headers=headers,
                timeout=180,
            )
            if data.status_code:
        ...
        try:
            if self.sf_access_token:
                ...
                data = requests.request(
                    "GET",
                    self.sf_url
                    + f"/services/data/v40.0/query/?q=select Id from Product2 where (LastModifiedDate > {sf_modified_date} AND IsActive != False) ORDER BY LastModifiedDate",
                    headers=headers,
                    timeout=180,
                )
                file_data = requests.request(
    ...
    def getSfQuoteLineData(
        self, quote_id=None, quote_dict=None, is_so=False, so_id=None
    ):
        if self.sf_access_token:
            headers = {}
            headers["Authorization"] = "Bearer " + str(self.sf_access_token)
                                                    ...
                                                    if calculated_tax:
                                                        ...
                                                        if account_tax:
                                                            ...
                                                            if not self.country_id:
                                                                raise UserError(
                                                                    _(
                                                                        "Please set the Country for Company"
                                                                    )
                                                                )
    ...
    def import_sf_quote(self):
        ...
        if self.sf_access_token:
            ...
            data = requests.request(
                "GET",
                self.sf_url
                + f"/services/data/v40.0/query/?q=select Id from quote where (LastModifiedDate > {sf_modified_date}) ORDER BY LastModifiedDate",
                headers=headers,
                timeout=180,
            )
            if data.status_code == 200:
                            ...
                            for r in recs.get("records"):
                                ...
                                if quote_data:
                                    ...
                                    quote_line_data = self.getSfQuoteLineData(
                                        quote_id=r.get("Id"), quote_dict=quote_data
                                    )
                                    if quote_line_data:
    ...
    def import_sf_so(self):
        ...
        if self.sf_access_token:
            ...
            data = requests.request(
                "GET",
                self.sf_url
                + f"/services/data/v39.0/query/?q=select Id from order where (LastModifiedDate > {sf_modified_date}) ORDER BY LastModifiedDate",
                headers=headers,
                timeout=180,
            )
            if data.status_code == 200:
    ...
    def create_odoo_sf_contact_dictionary(self, sf_contact_info):
        ...
        if sf_contact_info.get("MailingCountry", False):
            country_id = self.env["res.country"].search(
                [("name", "=", sf_contact_info.get("MailingCountry"))], limit=1
            )
            if country_id:
    ...
    def import_sf_accounts(self):
        if self.sf_access_token:
            ...
            company_data = requests.request(
                "GET",
                self.sf_url
                + f"""/services/data/v40.0/query/?q=select Id, LastModifiedDate from account where LastModifiedDate > {sf_modified_date} ORDER BY LastModifiedDate""",
                headers=headers,
                timeout=180,
            )
        ...
        try:
            if self.sf_access_token:
                ...
                data = requests.request(
                    "GET",
                    self.sf_url
                    + f"/services/data/v40.0/query/?q=select Id from Lead where (LastModifiedDate > {sf_modified_date}) ORDER BY LastModifiedDate",
                    headers=headers,
                    timeout=180,
                )
                if data:
                                    ...
                                    state_id = self.env["res.country.state"].search(
                                        [
                                            ...
                                            (
                                                "name",
                                                "=",
                                                lead_read.get("Address").get("state"),
                                            ),
                                            (
                                                "code",
                                                "=",
        ...
        try:
            if self.sf_access_token:
                ...
                data = requests.request(
                    "GET",
                    self.sf_url
                    + f"/services/data/v40.0/query/?q=select Id from Opportunity where (LastModifiedDate > {sf_modified_date}) ORDER BY LastModifiedDate",
                    headers=headers,
                    timeout=180,
                )
                if data:
                    ...
                    for rec in recs:
                        ...
                        if opportunity_read.get("Amount"):
                            opportunity_dict["expected_revenue"] = opportunity_read.get(
                                "Amount"
                            )
                        if opportunity_read.get("Probability"):
                            opportunity_dict["probability"] = opportunity_read.get(
                                "Probability"
                            )
                        ...
                        if opportunity_read.get("CloseDate"):
                            opportunity_dict["date_deadline"] = opportunity_read.get(
                                "CloseDate"
                            )

                        if opportunity_read.get("Description"):
                            opportunity_dict["description"] = opportunity_read.get(
                                "Description"
                            )

                        if opportunity_read.get("StageName"):
                            crm_stage = self.env["crm.stage"].search(
                                [("name", "=", opportunity_read.get("StageName"))],
                                limit=1,
                            )
        ...
        try:
            if self.sf_access_token:
                ...
                data = requests.request(
                    "GET",
                    self.sf_url
                    + f"/services/data/v40.0/query/?q=select Id from Contract where (LastModifiedDate > {sf_modified_date}) ORDER BY LastModifiedDate",
                    headers=headers,
                    timeout=180,
                )
                if data:
                    ...
                    for rec in recs:
                        ...
                        if contract_read.get("StartDate"):
                            contract_dict["contract_start_date"] = contract_read.get(
                                "StartDate"
                            )
                        if contract_read.get("ContractTerm"):
                            contract_dict["contacr_term_month"] = contract_read.get(
                                "ContractTerm"
                            )
                        if contract_read.get("Status"):
                            if contract_read.get("Status") == "Draft":
                    ...
                    for rec in recs:
                        ...
                        if event_read.get("EndDateTime"):
                            result2 = datetime.strptime(
                                event_read.get("EndDateTime")[0:19], "%Y-%m-%dT%H:%M:%S"
                            )
        ...
        try:
            if self.sf_access_token:
                ...
                data = requests.request(
                    "GET",
                    self.sf_url
                    + f"/services/data/v40.0/query/?q=select Id from Task where (LastModifiedDate > {sf_modified_date}) ORDER BY LastModifiedDate",
                    headers=headers,
                    timeout=180,
                )
                if data:
                    ...
                    for rec in recs:
                        ...
                        if activity_read.get("WhatId"):
                            result = self.sf_createOdooParentId_Activity(
                                activity_read.get("WhatId"), search_for="account"
                            )
                            if result:
                        ...
                        if not activity_read.get("WhoId") and not activity_read.get(
                            "WhatID"
                        ):
                            result = self.sf_createOdooParentId_Activity(
                                activity_read.get("OwnerId"), search_for="account"
                            )
                            if result:
                                res_partner = self.env["res.partner"].search(
                                    [("id", "=", result)], limit=1
