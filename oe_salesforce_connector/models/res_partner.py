...
class ResPartnerCustomization(models.Model):
    ...
    def sendDataToSalesforceForUpdate(self, dict):
        ...
        if sf_access_token:
            ...
            if result.status_code in [204]:
                ...
                raise UserError("Error Occurred While Updating" + result.text)
                return False
    ...
    def checkPartnerInSalesforce(self, odoo_partner_object):
        ...
        if sf_config:
            ...
            if sf_access_token:
                ...
                result = requests.request('GET', sf_config.sf_url + "/services/data/v40.0/query/?q=select Id from account where Name = '{}'".format(q_para), headers=headers)
                if result.status_code == 200:
                    ...
                    raise UserError("Error Occurred In Partner Search Request" + result.text)
                return False
    ...
    def sendDataToSalesforce(self, dict, record_type=False):
        ...
        if sf_access_token:
            ...
            if result.status_code in [200, 201]:
                ...
                raise UserError("Error Occurred While Exporting" + result.text + str(result.status_code))
                return False
        ...
        if result.status_code in (200, 201):
            ...
            sf_config.write({
                'salesforce_instance_line_ids': [(0, 0, {
                    ...
                    'date_time': datetime.now(),
                    'state': 'error',
                    'message': ('Enable to Export:- %s.' % (str(parsed_json[0].get('message'))))
                })]
            })
    ...
    def _scheduler_export_companies_to_sf(self):
        ...
        if sf_config.export_limit:
            companies = self.search([('x_is_updated', '=', False), ('id', 'not in', [1, 2, 3]), ('is_company', '=', True)], limit=sf_config.export_limit)
        else:
    ...
    def _scheduler_export_contacts_to_sf(self):
        ...
        if sf_config.export_limit:
            contacts = self.search([('x_is_updated', '=', False), ('id', 'not in', [1, 2, 3]), ('is_company', '=', False), ('type', '=', 'contact')], limit=sf_config.export_limit)
        else:
    ...
    def create_partner_delivery_address_in_sf(self, delivery_contact_data, is_from_cron=False):
        if is_from_cron:
            sf_config = self.env['salesforce.instance'].sudo().search([('is_default_instance', '=', True)], limit=1)
        else:
            sf_config = self.env['salesforce.instance'].sudo().search([('company_id', '=', self.env.company.id)], limit=1)
        ...
        try:
            ...
            result = requests.request('POST', sf_config.sf_url + endpoint, headers=headers, data=json.dumps(delivery_contact_data))
    ...
    def exportPartner_to_sf(self):
        ...
        for partner in self.browse(active_ids):
            if partner.is_company:
                ...
                if partner.x_salesforce_id:
                    partner.update_partner_in_sf(sf_company_dict)
                    delivery_addresses = partner.child_ids.filtered(lambda l: l.type == 'delivery' and l.x_salesforce_id is False)
                    if delivery_addresses:
                        for dla in delivery_addresses:
                            sf_contact_delivery_address_dict = dla.create_contact_sf_delivery_address_dict()
                            dla.create_partner_delivery_address_in_sf(sf_contact_delivery_address_dict)
                else:
                    partner.create_partner_in_sf(sf_company_dict)
