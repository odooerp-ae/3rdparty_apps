...
class CRMOpportunity(models.Model):
    ...
    x_salesforce_exported_oppo = fields.Boolean(
        "Exported To Salesforce from opportunity", default=False, copy=False
    )
    x_salesforce_id_oppo = fields.Char("Salesforce Id from Opportunity", copy=False)
    x_is_updated_oppo = fields.Boolean(
        "x_is_updated_from_opportunity", default=False, copy=False
    )
    sf_status_oppo = fields.Selection(
        [
        ...
        if sf_access_token:
            ...
            if self.x_salesforce_id_oppo:
                ...
                res = requests.request(
                    "PATCH",
                    sf_config.sf_url + endpoint + "/" + self.x_salesforce_id_oppo,
                    headers=headers,
                    data=payload,
                    timeout=180,
                )
    ...
    def _scheduler_export_opportunity_to_sf(self):
        ...
        if sf_config.export_limit:
            opportunities = self.search(
                [("type", "=", "opportunity")], limit=sf_config.export_limit
            )
        else:
        ...
        for opportunity in opportunities:
            try:
                ...
                _logger.error(
                    "Oops Some error in  exporting Opportunity to SALESFORCE %s", e
                )
