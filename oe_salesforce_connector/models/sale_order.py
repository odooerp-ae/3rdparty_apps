...
class SaleOrderCust(models.Model):
    ...
    sale_order_salesforce_id = fields.Char('Sale Order Salesforce Id', copy=False)
    ...
    def sendDataToSf(self, order_dict, is_cron=False):
        if is_cron:
            ...
            sf_config = self.env['salesforce.instance'].sudo().search([('company_id', '=', self.env.company.id)],
                                                                      limit=1)
        ...
        if sf_access_token:
            ...
            if self.sale_order_salesforce_id:
                ...
                res = requests.request('PATCH', sf_config.sf_url + endpoint + '/' + self.sale_order_salesforce_id,
                ...
                if res.status_code in (200, 201, 204):
                    ...
                    _logger.info('Sale order updated in salesforce %s', self.sale_order_salesforce_id)
                ...
                if res.status_code in (200, 201):
                    ...
                    _logger.info('Sale order created in salesforce %s', parsed_resp.get('id'))
        ...
        if sf_access_token:
            ...
            if line_sf_id:
                ...
                if res.status_code in [200, 201]:
                    ...
                    _logger.info('Sale order line created in odoo %s', order_line.sale_order_line_salesforce_id)
        ...
        if result:
            if self.order_line:
                for line in self.order_line:
                    if line.sale_order_line_salesforce_id and line.sale_order_line_updated:
                        delete_so_line = self.delete_sale_order_line_to_salesforce(line,
                        ...
                        if delete_so_line:
                            ...
                            PricebookEntryData = requests.request('GET',
                                                                  sf_config.sf_url + "/services/data/v40.0/query/?q=select Id,UnitPrice from PricebookEntry where  Product2Id='{}'".format(
                                                                      order_line_dict['Product2Id']), headers=headers)
                        ...
                        line_id = line.id
                        if order_line_dict:
                            result = self.sendLineDataToSf(order_line_dict, line_sf_id, line_id, is_cron=is_from_cron)
                        self.x_salesforce_exported = True
        ...
        if sf_config.sf_access_token:
            ...
            if sf_access_token:
                ...
                if res.status_code == 200 or res.status_code == 201 or res.status_code == 204:
                    _logger.info('Sale order line is deleted from SALESFORCE===========================')
    ...
    def _scheduler_export_orders_to_sf(self):
        ...
        if sf_config.export_limit:
            orders = self.search([('state', 'in', ('sale', 'done'))], limit=sf_config.export_limit)
        else:
...
class SaleOrderLine(models.Model):
    ...
    x_salesforce_id = fields.Char('Salesforce Id', copy=False)
    sale_order_line_salesforce_id = fields.Char('Sale Order Line Salesforce Id', copy=False)
    x_salesforce_exported = fields.Boolean('Exported To Salesforce', default=False, copy=False)
    x_is_updated = fields.Boolean('x_is_updated', default=False, copy=False)
    ...
    def unlink(self):
        ...
        if sf_config.sf_access_token:
            ...
            for record in self:
                if (record.x_salesforce_id or record.sale_order_line_salesforce_id) and (
                        record.order_id.state == 'draft' or record.order_id.state == 'cancel'):
                    endpoint = "/services/data/v52.0/sobjects/QuoteLineItem/" + str(record.x_salesforce_id)
                    res = requests.request('DELETE', sf_config.sf_url + endpoint, headers=headers)
                    if res.status_code == 200 or res.status_code == 201 or res.status_code == 204:
                        _logger.info('Quote Line is deleted from SALESFORCE===========================')
