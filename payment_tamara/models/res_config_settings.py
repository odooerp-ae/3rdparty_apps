...
class ResConfigSettings(models.TransientModel):
    ...
    data_exclude_products = fields.Many2many(
        "product.product", string="Exclude Products", related="company_id.data_exclude_products", readonly=False
    )
    ...
    def _onchange_data_installment_maximum_amount(self):
        if self.data_installment_maximum_amount < self.data_installment_minimum_amount:
            raise UserError(
                _("The Installation maximum amount should not be less than the Installation minimum amount.")
            )
    ...
    def _onchange_data_installment_minimum_amount(self):
        if self.data_installment_minimum_amount > self.data_installment_maximum_amount:
            raise UserError(
                _("The Installation minimum amount should not be greater than the Installation maximum amount.")
            )
